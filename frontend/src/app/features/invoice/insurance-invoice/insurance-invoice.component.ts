...
export class InsuranceInvoiceComponent implements OnInit {
  ...
  checkInsuranceMapping() {
    if (this.companies && this.companies.length > 0 && this.form && this.form.inv_insurance_company) {
      this.onInsuranceChange();
    }
  }
  ...
  printInvoice() {
    ...
    if (environment.newPDFInvoicePrint) {
      const pdfTitle = `${this.form.inv_no || this.invoiceId} - Insurance Invoice`;
